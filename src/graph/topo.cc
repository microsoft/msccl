...
ncclResult_t ncclTopoConnectNodes(struct ncclTopoNode *node, struct ncclTopoNode *remNode, int type, float width)
{
  ...
  {
    if ((link - 1)->width >= linkSave.width)
      break;
...
ncclResult_t ncclTopoPrint(struct ncclTopoSystem *s)
{
  ...
  for (int n = 0; n < s->nodes[CPU].count; n++)
    NCCLCHECK(ncclTopoPrintRec(s->nodes[CPU].nodes + n, NULL, line, 0));
  INFO(NCCL_GRAPH, "==========================================");
...
ncclResult_t ncclTopoAddNet(struct ncclXmlNode *xmlNet, struct ncclTopoSystem *system, struct ncclTopoNode *nic)
{
  ...
  if (xmlGetAttrFloat(xmlNet, "latency", &net->net.latency) != ncclSuccess)
    net->net.latency = 0;
  ...
  if (cpu->cpu.arch == NCCL_TOPO_CPU_ARCH_X86)
  {
    ...
    else if (cpu->cpu.vendor == NCCL_TOPO_CPU_VENDOR_ZHAOXIN)
    {
      ...
      if (familyId == 7 && modelId == 0x5B)
        cpu->cpu.model = NCCL_TOPO_CPU_TYPE_YONGFENG;
    }
  }
...
ncclResult_t ncclTopoGetLocalNet(struct ncclTopoSystem *system, int rank, int *id)
{
  ...
  for (int n = 0; n < system->nodes[NET].count; n++)
  {
    ...
    if (path->width == maxWidth && path->type == minType)
            ...
            for (int st = 0; st < threadblockNode->nSubs; st++)
            {
              ...
              if (strcmp(stepNode->name, "step") == 0)
              {
                ...
                if (transferType != -1)
                {
                  ...
                  if (checkSrc)
                    NCCLCHECK(mscclCheckBufferBounds(msccltran->srcbuffer, msccltran->srcoffset, nInputChunks, nOutputChunks, nScratchChunks));
                  if (checkDst)
                    NCCLCHECK(mscclCheckBufferBounds(msccltran->dstbuffer, msccltran->dstoffset, nInputChunks, nOutputChunks, nScratchChunks));
                  ...
                  {
                    if (oldReductionDstBuffer == -1)
                    { // if this is the first reduction
