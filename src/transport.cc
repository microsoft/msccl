...
static ncclResult_t selectTransport(struct ncclComm *comm, struct ncclTopoGraph *graph, struct ncclConnect *connect, int channelId, int peer, int connIndex, uint32_t mscclPreSelectedType, int *transportType)
{
  ...
  { // 0 is default
    ...
    if (ret)
    {
      ...
      if (transportType)
        *transportType = mscclPreSelectedType - 1;
      return ncclSuccess;
    }
    else
    {
...
void dumpData(struct ncclConnect *data, int ndata)
{
  for (int n = 0; n < ndata; n++)
  {
    ...
    for (int i = 0; i < sizeof(struct ncclConnect); i++)
      printf("%02x", d[i]);
    printf("\n");
  }
}
...
ncclResult_t ncclTransportP2pSetup(struct ncclComm *comm, struct ncclTopoGraph *graph, int connIndex, int *highestTransportType /*=NULL*/)
{
  ...
  for (int i = 1; i < comm->nRanks; i++)
  {
    ...
    else
    {
      ...
      if (sendChannels)
        NCCLCHECK(bootstrapSend(comm->bootstrap, sendPeer, bootstrapTag, sendData, sizeof(struct ncclConnect) * sendChannels));
      if (sendChannels)
        NCCLCHECK(bootstrapRecv(comm->bootstrap, sendPeer, bootstrapTag, sendData, sizeof(struct ncclConnect) * sendChannels));
      if (recvChannels)
  ...
  if (highestTransportType != NULL)
    *highestTransportType = highestType;
...
int ncclTransportCollNetSetup(struct ncclComm *comm, struct ncclTopoGraph *collNetGraph, struct ncclChannel *channel, int masterRank, int masterPeer, int collNetGraphChannelId, int type)
{
  ...
  { // send side : copy in connect info received from peer recv master
    if (isMaster)
      memcpy(masterConnects + rankInCollNet, &(sendrecvExchange.connect), sizeof(struct ncclConnect));
  }
  // connect
  if (isMaster && support)
  {
...
ncclResult_t ncclTransportCollNetCheck(struct ncclComm *comm, int collNetSetupFail)
{
  ...
  {
    if (comm->localRank == 0)
      WARN("Cannot initialize CollNet, using point-to-point network instead");
    return ncclSystemError;
  }
  return ncclSuccess;
}
  ...
  for (int r = 0; r < comm->nChannels; r++)
  {
    ...
    for (int b = 0; b < NCCL_MAX_CONNS; b++)
    {
      ...
      if (send->transportResources && send->transportComm)
        NCCLCHECK(send->transportComm->free(send));
    ...
    for (int b = 0; b < NCCL_MAX_CONNS; b++)
    {
      ...
      if (recv->transportResources && recv->transportComm)
        NCCLCHECK(recv->transportComm->free(recv));
