...
private:
 ...
 public:
  ...
  __device__ void recvReduceCopySend(intptr_t inpIx, intptr_t outIx, int eltN, bool postOp=false) {
    ...
    if (tid == 0) {
      NpKit::CollectGpuEvent(NPKIT_EVENT_LOCAL_COPY_ENTRY, eltN*sizeof(T), 0, clock64(),
          ncclShmem.comm.npKitEventCollectContexts + npKitCtxIdx);
    }
#endif
    ...
    if (tid == 0) {
      NpKit::CollectGpuEvent(NPKIT_EVENT_LOCAL_COPY_EXIT, eltN*sizeof(T), 0, clock64(),
          ncclShmem.comm.npKitEventCollectContexts + npKitCtxIdx);
    }
#endif
  }
  ...
  __device__ void reduce(T** srcs, int nsrcs, T** dsts, int ndsts, int eltN){
    ...
    if (tid == 0) {
      NpKit::CollectGpuEvent(NPKIT_EVENT_REDUCE_ENTRY, eltN*sizeof(T), 0, clock64(),
          ncclShmem.comm.npKitEventCollectContexts + npKitCtxIdx);
    }
#endif
    if (nsrcs == 1) {
    ...
    if (tid == 0) {
      NpKit::CollectGpuEvent(NPKIT_EVENT_REDUCE_EXIT, eltN*sizeof(T), 0, clock64(),
          ncclShmem.comm.npKitEventCollectContexts + npKitCtxIdx);
    }
#endif
  }
